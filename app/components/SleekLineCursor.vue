...
class Line {
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    const eLast = this.nodes[this.nodes.length - 2];
    const tLast = this.nodes[this.nodes.length - 1];
    if (!eLast || !tLast) return;
    ctx.quadraticCurveTo(eLast.x, eLast.y, tLast.x, tLast.y);
    ctx.stroke();
    ctx.closePath();
  }
}
