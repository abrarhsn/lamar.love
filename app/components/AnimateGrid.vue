...
onMounted(() => {
  card.value?.forEach((el, i) => {
    ...
    const removeClasses = useDebounceFn(() => removeCardClasses(el, adjacentCards), 50);
    ...
    <div
      ...
      :class="
        cn(
          ...
          props.cards.length < 4 ? `grid-cols-${props.cards.length}` : 'grid-cols-3',
        )
      "
    >
      <div
        ...
        :key="index"
        ref="card"
        class="card block rounded border border-transparent px-3 py-5 transition-all duration-75"
...
.card-raised-small {
  ...
  transform: scale(1.02) translateX(-2px) translateY(-2px) translateZ(0);
  animation: text-glow-small 0.7s alternate infinite ease-in-out;
...
.card-raised-big {
  ...
  transform: scale(1.06) translateX(-8px) translateY(-8px) translateZ(6px);
  animation: text-glow 0.7s alternate infinite ease-in-out;
  transition-delay: 0ms;
}
